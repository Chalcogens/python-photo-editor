...
def brighten(image, factor):
    # adjust brightness = adjusting the value of each channel
    # increase channel value to brighten, decrease to darken
    x_pixels, y_pixels, num_channels = image.array.shape

    #create new array instead of mutating old one
    new_im = Image(x_pixels=x_pixels, y_pixels=y_pixels, num_channels=num_channels)

    # non-vectorised for loops adjust pixel by pixel
    for x in range(x_pixels):
        for y in range(y_pixels):
            for c in range(num_channels):
                new_im.array[x,y,c] = image.array[x,y,c] * factor

    # alternative (faster) is vectorised version:
...
def blur(image, kernel_size):
    ...
    for x in range(x_pixels):
        for y in range(y_pixels):
            for c in range(num_channels):
                total = 0
                for x_i in range(max(x-neighbour_range,0), min(x_pixels-1,x+neighbour_range)+1):
                    for y_i in range(max(y-neighbour_range, 0), min(y_pixels - 1, y+neighbour_range) + 1):
...
def apply_kernel(image, kernel):
    ...
    x_pixels, y_pixels, num_channels = image.array.shape
    new_im = Image(x_pixels=x_pixels, y_pixels=y_pixels, num_channels=num_channels)
    neighbor_range = kernel.shape[0] // 2  #tells us how many neighbours to look for
    for x in range(x_pixels):
        for y in range(y_pixels):
            for c in range(num_channels):
                total = 0
...
if __name__ == '__main__':
    ...
    #edge detector from combining sobel_x and sobel_y
